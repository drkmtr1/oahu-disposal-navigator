...
- OD-006: Operational baseline set 2026-09-08: Vercel limits `/api/disposal-options` to 60 requests per 60-second fixed window per IP; `drkmtr1` is the recovery owner and the recovery-time objective is 24 hours. Supabase daily physical backup availability is verified. The destructive logical export/restore drill remains a BL-013 release gate until separately authorized; this does not authorize restoring production or creating a temporary restore project.
