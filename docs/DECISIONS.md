...
- OD-006: Operational baseline set 2026-09-08: Vercel limits `/api/disposal-options` to 60 requests per 60-second fixed window per IP; `drkmtr1` is the recovery owner and the recovery-time objective is 24 hours. Supabase daily physical backup availability is verified. The project owner removed the destructive logical export/restore drill from V1 release requirements; the drill remains an optional future operational exercise and does not authorize restoring production or creating a temporary restore project.
