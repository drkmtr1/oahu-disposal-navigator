...
| R-005 | Technically working UI confuses residents with lower technical familiarity. | UX/product | High | Medium | one task/four states/plain language, automated accessibility evidence, safe recovery, frozen resident-task benchmark | Failure of the expected-state, source-association, or critical-safety benchmark gate creates a blocking item; reported limits remain explicit |
...
| R-011 | Abuse creates denial of service. | Security/operations | Medium | Medium | body/input limits, bounded application limiter, no retry loops; distributed Vercel control verified in BL-013 | Threshold breach tightens platform rate control; AI remains disabled |
...
| R-015 | Evaluation set is too small/biased and inflates performance. | Evaluation | Medium | Medium | frozen holdout, critical cases, failure analysis, raw counts/limits | Production behavior or authorized future research reveals a material gap and triggers dataset revision |
| R-016 | Non-participant benchmark is presented as proof of human usability or representative resident outcomes. | Evaluation/portfolio | Medium | High | explicit claim boundary, publish case composition/raw outcomes, retain separate accessibility evidence | Claims exceeding the benchmark evidence are corrected |
...
| R-019 | Public launch implies government endorsement. | Trust/legal | Medium | Medium | clear independent-project language and official-source attribution | Release review or resident feedback prompts content correction |
...
The maintainer owns risk review. Reassess at source freeze, AI gate, resident-task benchmark changes, and pre-production. High residual safety/security/accessibility risks block release.
