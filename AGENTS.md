...
Current lifecycle status: Stage 1 and BL-001 through BL-013 are merged to `main`, with BL-013 squash-merged at `cf340e6`; conditional BL-010 was closed as not justified. ADR-012 removed recruited-participant and consent requirements while preserving explicit non-participant evidence limits. The application-specific Supabase project and Vercel production deployment are live, with documented backup availability, recovery ownership/objective, application rollback, and forward-only data/schema correction. BL-014 V1 release audit is complete and its `v1.0.0` tag is created from the merged release-audit commit. The live-data path is exercised by Docker-backed CI because this workstation has no compatible local runtime. See `docs/BL-013_PRODUCTION_RELEASE.md` and `docs/BL-014_V1_RELEASE_AUDIT.md` for current evidence and limits.
